function [model] = svmatch(model, data, mdl, qmdl)
...
% [foo, objconfus_m] = max(objconfus');
% objconfus_o = zeros(length(objconfus));
% for i = 1:length(objconfus);
%    objconfus_o(i,objconfus_m(i)) = 1; 
% end
% 
% objconfus = objconfus_o;
