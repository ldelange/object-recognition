...
for i = 1:length(robjects)
    ...
    % select linear spaced data.views for this instance using band 1
    views_ = round(linspace(1,length(rgbd_dataset{robjects(i)}.instances{inst_,1}),data.views(end)));
        
    % write path to training objects/views
    data.dataset{end+1} = rgbd_dataset{robjects(i)}.instances{inst_,1}(views_)';
